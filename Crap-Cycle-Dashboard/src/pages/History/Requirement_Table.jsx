...
export default function Requirement_TableHistory() {
    ...
    React.useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axios.get(import.meta.env.VITE_BACKEND_URL+'/api/requirement/complete-requirement');
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {displayedData.map((row, index) => (
                                <tr key={index} className="hover:bg-gray-50">
                                    ...
                                    <td className="py-2 px-4">Rs {parseFloat(row.bided_quantity).toFixed(2)}</td>
